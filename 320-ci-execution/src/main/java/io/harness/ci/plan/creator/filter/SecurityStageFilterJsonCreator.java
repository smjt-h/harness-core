...
public class SecurityStageFilterJsonCreator extends GenericStageFilterJsonCreator {
  ...
  private void validateStage(StageElementConfig stageElementConfig) {
    ...
    if (infrastructure.getType() == Infrastructure.Type.VM) {
      ValidationUtils.validateVmInfraDependencies(securityStageConfig.getServiceDependencies().getValue());
    }
  }
  ...
  public Set<EntityDetailProtoDTO> getReferredEntities(
    ...
    SecurityStageConfig securityStage = (SecurityStageConfig) stageElementConfig.getStageType();
    if (securityStage.getInfrastructure() == null) {
      throw new CIStageExecutionException("Input infrastructure is not set");
    } else {
      if (securityStage.getInfrastructure().getType() == KUBERNETES_DIRECT) {
        K8sDirectInfraYaml k8sDirectInfraYaml = (K8sDirectInfraYaml) securityStage.getInfrastructure();
