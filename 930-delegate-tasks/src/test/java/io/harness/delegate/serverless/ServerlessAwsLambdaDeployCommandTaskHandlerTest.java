...
public class ServerlessAwsLambdaDeployCommandTaskHandlerTest extends CategoryTest {
    ...
    private ServerlessCommandRequest serverlessCommandRequest = ServerlessDeployRequest.builder().timeoutIntervalInMin(timeout)
            .serverlessInfraConfig(serverlessInfraConfig).serverlessManifestConfig(serverlessManifestConfig)
            .serverlessDeployConfig(serverlessDeployConfig).accountId(accountId).build();
    ...
    private ServerlessCliResponse deployServerlessCliResponse = ServerlessCliResponse.builder().commandExecutionStatus(
    ...
    public void executeTaskInternalTest() throws Exception{
        ...
        doReturn(deployLogCallback).when(serverlessTaskHelperBase).getLogCallback(iLogStreamingTaskClient, ServerlessCommandUnitConstants
                .deploy.toString(), true, commandUnitsProgress);
        ...
        doReturn(deployServerlessCliResponse).when(serverlessAwsCommandTaskHelper).deploy(serverlessClient, serverlessDelegateTaskParams,
                deployLogCallback, (ServerlessAwsLambdaDeployConfig) serverlessDeployConfig, (ServerlessAwsLambdaInfraConfig) serverlessInfraConfig,
                serverlessCommandRequest.getTimeoutIntervalInMin() * 60000);
        ...
        ServerlessAwsLambdaDeployResult serverlessAwsLambdaDeployResult =
                ServerlessAwsLambdaDeployResult.builder().service(service).region(region).stage(stage).previousVersionTimeStamp(
                        previousVersionTimeStamp).functions(serverlessAwsLambdaFunctionsList).build();

        ServerlessDeployResponse serverlessDeployResponse = (ServerlessDeployResponse) serverlessAwsLambdaDeployCommandTaskHandler.executeTaskInternal(serverlessCommandRequest,
